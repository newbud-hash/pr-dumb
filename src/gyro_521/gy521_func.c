...
char read_data(const struct gy_target_details *gy_target, unsigned char gy_reg, unsigned char bytes, unsigned char *raw_buffer)
{	
	if(!gy_target || !gy_target->bus_details)
		return INVARG;
...
{
	if(!gy_target_details || !receive_buffer)
		return INVARG;
...
char temperature_read(const struct gy_target_details *gy_target_details, float *temperature_buffer)
{
	unsigned char raw_temp[2] = {0};
	short temp_receive;
	char ret;
	
	if(!gy_target_details || !temperature_buffer)
		return INVARG;
